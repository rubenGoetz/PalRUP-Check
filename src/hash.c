...
bool realloc_table(struct hash_table* ht) {
    ...
    ht->data = (struct hash_table_entry*) palrup_utils_calloc(new_capacity, sizeof(struct hash_table_entry));
...
struct hash_table* hash_table_init(int log_init_capacity) {
    struct hash_table* ht = palrup_utils_malloc(sizeof(struct hash_table));
    ht->capacity = 1<<log_init_capacity;
    ht->size = 0;
    ht->max_size = (u64) (ht->capacity >> 1);
    ht->growth_factor = 2;
    ht->data = (struct hash_table_entry*) palrup_utils_calloc(ht->capacity, sizeof(struct hash_table_entry));
    return ht;
}
