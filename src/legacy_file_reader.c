...
void fill_buffer(struct legacy_file_reader* reader) {
    u64 read_size;
...
struct legacy_file_reader* legacy_file_reader_init(u64 buffer_size_bytes, FILE* file, int pal_id) {
    struct legacy_file_reader* reader = (struct legacy_file_reader*)palrup_utils_malloc(sizeof(struct legacy_file_reader));
...
void legacy_file_reader_seek(u64 byte_pos, struct legacy_file_reader* reader) {
    ...
    if (loaded_start <= searched_byte && searched_byte < loaded_end){ // seeked position is loaded in buffer
        ...
        legacy_file_reader_skip_bytes(searched_byte - current_pos, reader);
    }
...
int legacy_file_reader_read_int(struct legacy_file_reader* reader){
    int res;
    if (legacy_file_reader_check_bounds(sizeof(int), reader)){
        res = *((int*)(reader->pos));
...
void legacy_file_reader_read_ints(int* data, u64 nb_ints, struct legacy_file_reader* reader){
    if (legacy_file_reader_check_bounds(nb_ints * sizeof(int), reader)){
...
u64  legacy_file_reader_read_ul(struct legacy_file_reader* reader){
    u64 res;
    if (legacy_file_reader_check_bounds(sizeof(u64), reader)){
        res = *((u64*)(reader->pos));
...
void legacy_file_reader_read_uls(u64* data, u64 nb_uls, struct legacy_file_reader* reader){
    if (legacy_file_reader_check_bounds(nb_uls * sizeof(u64), reader)){
...
char legacy_file_reader_read_char(struct legacy_file_reader* reader){
    char res;
    legacy_file_reader_check_bounds(sizeof(char), reader); // makes shure that the char is in the buffer
    res = *(reader->pos);
    reader->pos += 1;
...
void legacy_file_reader_read_vbl_ints(int* data, u64 nb_ints, struct legacy_file_reader* reader) {
    for (size_t i = 0; i < nb_ints; i++)
        data[i] = legacy_file_reader_read_vbl_int(reader);
}
...
inline char legacy_file_reader_read_vbl_char(struct legacy_file_reader* reader) {
    return legacy_file_reader_read_char(reader);
}
